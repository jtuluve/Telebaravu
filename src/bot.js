...
bot.on("message:text", async (ctx) => {
  dbget(ctx.message.from.id, async (row) => {
    let msg = await bot.api.sendMessage(ctx.message.from.id, "It will take some time for me to generate png. Please wait..😇")
      ...
      axios.get(`https://tulu-png-api2.glitch.me/image?text=${txt}&font=${font}&color=${color}`)
        .then(async (response) => {
          await ctx.replyWithDocument(new InputFile(new URL(response.data.url),"image.png"));
